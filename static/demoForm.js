...
app.controller('MainCtrl', function ($scope, SendCode) {
  ...
  $scope.textData ="this data";
  $scope.formData = {};
  $scope.sendCode = function(){
      var codeData = document.getElementById("code");
      console.log("code data " );
     $scope.renderedCode = SendCode.renderCode("the test data");
  };
});
...
app.factory('SendCode',['$http', function($http, $scope){
    return {
        renderCode : function (data) {
            var SendCodeReq = {method: 'POST', url: 'http://localhost:5858/checkService', data:data};
