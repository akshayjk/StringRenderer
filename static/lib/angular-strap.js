        ...
        if (controller) {
          ...
          controller.$parsers.unshift(function (viewValue) {
            ...
            if (type === 'string') {
              ...
              //return type === 'string' ? new Date(viewValue).getTime() : $.fn.datepicker.DPGlobal.parseDate(viewValue, $.fn.datepicker.DPGlobal.parseFormat(format), language);
            } else {
          ...
          controller.$render = function ngModelRender() {
            ...
            if (!controller.$viewValue || controller.$viewValue == null)
              element.val('');
...
angular.module("$strap.directives").factory("$modal", [
  "$rootScope", "$compile", "$http", "$timeout", "$q", "$templateCache", "$strapConfig", function($rootScope, $compile, $http, $timeout, $q, $templateCache, $strapConfig) {
    var ModalFactory;
    ModalFactory = ModalFactory = function(config) {
      var Modal;
      Modal = function(config) {
        var onSuccess, options, scope, templateUrl;
        ...
        return $q.when($templateCache.get(templateUrl) || $http.get(templateUrl, {
          ...
          return res.data;
        })).then(onSuccess = function(template) {
          var $modal, id;
          ...
          angular.forEach(["show", "shown", "hide", "hidden"], function(name) {
            return $modal.on(name, function(ev) {
              return scope.$emit("modal-" + name, ev);
            });
          });
          $modal.on("shown", function(ev) {
            return $("input[autofocus]", $modal).first().trigger("focus");
          });
  ...
  "$q", "$modal", function($q, $modal) {
    ...
    return {
      restrict: "A",
      scope: true,
      link: postLink = function(scope, iElement, iAttrs, controller) {
        ...
        angular.forEach(["modalClass", "backdrop", "keyboard"], function(key) {
          if (angular.isDefined(iAttrs[key])) {
            return options[key] = iAttrs[key];
          }
        });
        return $q.when($modal(options)).then(onSuccess = function(modal) {
          return iElement.attr("data-target", "#" + modal.attr("id")).attr("data-toggle", "modal");
        });
      }
    };
  }
